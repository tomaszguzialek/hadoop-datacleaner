...
public class ConfigurationSerializer {
    ...
    public static AnalyzerBeansConfiguration deserializeAnalyzerBeansDatastores(String datastoresInput) {
        ...
        for (Map.Entry<String, List<TableDataProvider<?>>> mapEntry : tablesMap.entrySet()) {
            ...
            Datastore datastore = new PojoDatastore(datastoreName, schemaName, tableDataProviders);
        ...
        // ClasspathScanDescriptorProvider descriptorProvider = new
        ...
        descriptorProvider.addAnalyzerBeanDescriptor(Descriptors.ofAnalyzer(ValueDistributionAnalyzer.class));

        return new AnalyzerBeansConfigurationImpl().replace(datastoreCatalog).replace(descriptorProvider);
    }

    public static AnalysisJob deserializeAnalysisJobFromXml(String analysisJobXml,
        ...
        return jobReader.read(new ByteArrayInputStream(analysisJobXml.getBytes()));
    }
    ...
    public static String serializeAnalyzerBeansConfigurationDataStores(
        ...
        for (String datastoreName : analyzerBeansConfiguration.getDatastoreCatalog().getDatastoreNames()) {
            ...
            Schema schema = datastore.openConnection().getDataContext().getDefaultSchema();
    ...
    public static String serializeAnalysisJobToXml(AnalyzerBeansConfiguration analyzerBeansConfiguration,
            AnalysisJob analysisJob) {
        JaxbJobWriter jaxbJobWriter = new JaxbJobWriter(analyzerBeansConfiguration);
    ...
    public static Analyzer<?> initializeAnalyzer(String analyzerKey, AnalyzerBeansConfiguration analyzerBeansConfiguration, AnalysisJob analysisJob) {
        ...
        Analyzer<?> analyzer = ReflectionUtils.newInstance(analyzerJob.getDescriptor().getComponentClass());
        LifeCycleHelper lifeCycleHelper = new LifeCycleHelper(
                analyzerBeansConfiguration.getInjectionManager(analysisJob), true);
        lifeCycleHelper.assignConfiguredProperties(analyzerJob.getDescriptor(), analyzer,
